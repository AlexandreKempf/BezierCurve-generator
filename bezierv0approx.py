...
class LUT(QtGui.QWidget):
    ...
    def initUI(self):
        ...
        self.bezierCoord = np.array([[[15,15],[15,15],[50,50]], [[100,100],[150,100],[150,120]], [[200,200],[250,250],[250,250]]], dtype=np.float)
        self.movepoint=False

    def getBezierCurveCoord(self):
        points = np.reshape(self.bezierCoord,(int(self.bezierCoord.size/2),2))[1:-1]
        return bezier_curve_range(1600,points)
...
#        qp.setPen(pen)
    ...
    def drawBezierCurve(self, qp):
        coord = self.getBezierCurveCoord()
        for i in coord:
    ...
    def paintEvent(self, e):
        ...
        self.bezierCoord[:,:,0] -= np.min(self.bezierCoord[:,1,0])
        self.bezierCoord[:,:,1] -= np.min(self.bezierCoord[:,1,1])
        self.bezierCoord[:,:,0] = self.bezierCoord[:,:,0] / np.max(self.bezierCoord[:,1,0])
        self.bezierCoord[:,:,1] = self.bezierCoord[:,:,1]  / np.max(self.bezierCoord[:,1,1])
